...
export default async function ClientJobTankPage(props: PageProps) {
  ...
  // Server Action for requesting intro (Internal Logic - Unchanged)
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "40px 20px" }}>
        ...
        <div style={{ background: "white", padding: "30px", borderRadius: "20px", border: "1px solid #e2e8f0", marginBottom: "40px" }}>
           ...
           <div>
              ...
              {/* FIX: whiteSpace: "pre-wrap" preserves the admin formatting */}
              <div style={{ 
                background: "#f8fafc", 
                padding: "20px", 
                borderRadius: "12px", 
                ...
                return (
                  <tr key={item.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                    <td style={{ padding: "24px 20px" }}>
                       {/* FIX: Removed the arrow beside the name */}
                       {isUnlocked ? (
                       ...
                       {/* FIX: Emojis removed from chips for professional SAS look */}
                    ...
                    <td style={{ padding: "20px" }}>
                       <span style={{ 
                         ...
                         fontSize: '12px', 
                         fontWeight: 800, 
                         background: isUnlocked ? '#dcfce7' : isPending ? '#fef9c3' : '#f1f5f9', 
                         color: isUnlocked ? '#166534' : isPending ? '#ca8a04' : '#64748b',
                         textTransform: 'uppercase'
                       }}>
                         {isUnlocked ? "Unlocked" : isPending ? "Pending" : "Locked"}
                       </span>
                    </td>
                    ...
                    <td style={{ padding: "20px" }}>
                       {isUnlocked ? (
                         <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                           {c.resume_url ? (
                             <a href={c.resume_url} target="_blank" style={{ color: accent, fontWeight: 700, textDecoration: "none", fontSize: '13px' }}>Resume</a>
                           ) : (
                             <span style={{ color: '#94a3b8', fontSize: '13px', fontStyle: 'italic' }}>No Resume</span>
                           )}
                           {card?.linkedin_url && (
                             <a href={card.linkedin_url} target="_blank" style={{ color: '#0a66c2', fontWeight: 700, textDecoration: "none", fontSize: '13px' }}>LinkedIn</a>
                           )}
                         </div>
                       ) : !isPending && (
